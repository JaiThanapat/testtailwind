...
const RecoverUser = () => {
  ...
  const fetchDisableUsers = () => {
    UserService.fetchDisableUsers().then((users) => {
      setUsers(users);
    });
  };
  const handleViewInfoStaff = (UserID: any) => {
    history.push({
      pathname: `/UserRecoverProfile/${UserID}`,
      state: { UserID: UserID },
    });
  };
  ...
  const handleRecoverUser = (accountId: any) => {
    const jsonAccountID = {
      accountId: accountId,
    };
    UserService.activateUser(jsonAccountID, AuthService.getAccessToken());
                                        ...
                                        <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-lg outline-none focus:outline-none">
                                          ...
                                          <div className="relative flex-auto p-6">
                                            <p className="my-4 text-lg leading-relaxed text-black ">
                                              คุณต้องการคืนสิทธิผู้ใช้งานคนนี้ออกใช่หรือไม่
                                            </p>
                                          </div>
                                          {/*footer*/}
