...
const StaffsManager = () => {
  ...
  const fetchStaffs = () => {
    StaffService.fetchStaffs().then((staffs) => {
      setStaff(staffs);
    });
  };
  const handleCreateNewStaff = () => {
    history.push("/NewStaffForm");
  };
  ...
  const handleViewInfoStaff = (staffID: any) => {
    history.push({
      pathname: `/StaffProfile/${staffID}`,
      state: { staffID: staffID },
    });
  };
  const handleEditStaff = (staffID: any) => {
    history.push({
      pathname: `/EditStaff/${staffID}`,
      state: { staffID: staffID },
    });
  };
  ...
  const handleDeteleStaff = (staff: any) => {
    StaffService.DeletedStaff(staff, AuthService.getAccessToken());
                            ...
                            <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                              <div className="flex items-start space-x-3">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    handleEditStaff(person.staff_id);
                                  }}
                                >
                                  แก้ไข
                                </button>
